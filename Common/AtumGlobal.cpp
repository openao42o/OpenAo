...
void SetParamFactor_DesParam( CParamFactor &paramFactor, int nDestParam, float fDestValue)
{
	...
	//case DES_GRADUAL_DP_UP:				// ¿¡³ÊÁö·ù, ÇöÀç DP¸¦ ÀÏÁ¤ ½Ã°£(ITEM.Time¿¡ ÀúÀå)µ¿¾È Á¡ÁøÀûÀ¸·Î VALUE¸¸Å­ ¿Ã·ÁÁÜ, Áß°£¿¡ ºÎ½ºÅÍ ÄÑ°Å³ª °ø°Ý ¹ÞÀ¸¸é cancel, check: Ãß°¡µÊ
	...
	case DES_GROUNDMODE	:				// Æø°Ý¸ðµå
	case DES_SIEGEMODE	:				// ½ÃÁî¸ðµå
	case DES_BULLET_01	:				// (*) ºö¼Ò¸ðÃÑ¾Ë·® 01, check: Ãß°¡µÊ
	case DES_BULLET_02	:				// (*) ºö¼Ò¸ðÃÑ¾Ë·® 02, check: Ãß°¡µÊ
	case DES_PRIMARY_WEAPON	:			// 1Çü ¹«±â, ¹ü¿ë
	case DES_SECONDARY_WEAPON:			// 2Çü ¹«±â, ¹ü¿ë
	case DES_ALL_WEAPON	:				// 1,2Çü ¹«±â ¸ðµÎ, ¹ü¿ë
		break;
	case DES_OVERHITTIME_01:			// (*) ¿À¹öÈýÈ¸º¹½Ã°£ 01, check: Ãß°¡µÊ
//		paramFactor.pfm_TIME_01 += fDestValue;
		break;
	case DES_OVERHITTIME_02:			// (*) ¿À¹öÈýÈ¸º¹½Ã°£ 02, check: Ãß°¡µÊ
...
//		paramFactor.pfp_DODGE_PART += fDestValue;
	...
	case DES_HPRECOVERY	:				// ¿¡³ÊÁöÆ÷ÀÎÆ®È¸º¹·Â
		...
		break;
	case DES_MINATTACK_01:				// ÃÖ¼Ò °ø°Ý·Â 01, check: * 
		paramFactor.pfm_MINATTACK_01 += fDestValue;
		break;
	case DES_MAXATTACK_01:				// ÃÖ´ë °ø°Ý·Â 01, check: * 
		paramFactor.pfm_MAXATTACK_01 += fDestValue;
		break;
	case DES_MINATTACK_02:				// ÃÖ¼Ò °ø°Ý·Â 02, check: * 
		paramFactor.pfm_MINATTACK_02 += fDestValue;
		break;
	case DES_MAXATTACK_02:				// ÃÖ´ë °ø°Ý·Â 02, check: * 
		paramFactor.pfm_MAXATTACK_02 += fDestValue;
		break;
	case DES_ATTACKPROBABILITY_01:		// °ø°ÝÈ®·ü 01
		...
		break;
	case DES_ATTACKPROBABILITY_02:		// °ø°ÝÈ®·ü 02
		// 2010-07-28 by dgwoo È®·ü ¼ö½Ä º¯°æ (255 => 100%)
	...
	case DES_DEFENSEPROBABILITY_02:		// ¹æ¾îÈ®·ü 02
		...
		break;
	// 2010-07-28 by dgwoo È®·ü ¼ö½Ä º¯°æ (255 => 100%)
	case DES_SKILLPROBABILITY_01	:	// ½ºÅ³°ø°ÝÈ®·ü 01
	case DES_SKILLPROBABILITY_02	:	// ½ºÅ³°ø°ÝÈ®·ü 02
	...
	case DES_REATTACKTIME_02	:		// (*) ¸®¾îÅÃÅ¸ÀÓ 02 (- Áõ°¡)
		...
		break;
	case DES_ABRASIONRATE_01	:		// ¸¶¸ðÀ² 01
		paramFactor.pfp_ABRASIONRATE_01 += (Prob255_t)fDestValue;
		break;
	case DES_ABRASIONRATE_02	:		// ¸¶¸ðÀ² 02
	...
	case DES_MULTITAGET_02:				// ¸ÖÆ¼Å¸°Ù 02
		...
		break;
	case DES_EXPLOSIONRANGE_01:			// Æø¹ß¹Ý°æ 01
		paramFactor.pfp_EXPLOSIONRANGE_01 += fDestValue;
		break;
	case DES_EXPLOSIONRANGE_02:			// Æø¹ß¹Ý°æ 02
	...
	case DES_MULTINUM_02	:			// µ¿½Ã ¹ß»ç ¼ö 02
		...
		break;
	case DES_ATTACKTIME_01:				// Ã³À½ °ø°Ý ½ÃÀÇ Å¸ÀÓ 01 (- Áõ°¡)
		paramFactor.pfp_ATTACKTIME_01 += fDestValue;
		break;
	case DES_ATTACKTIME_02:				// Ã³À½ °ø°Ý ½ÃÀÇ Å¸ÀÓ 02 (- Áõ°¡)
	...
	case DES_ENGINE_BOOSTER_TIME_UP:	// ºÎ½ºÅÍ ½Ã°£ Áõ°¡
		...
		break;
	case DES_ENGINE_MAX_SPEED_UP:		// ¿£Áø ÀÏ¹Ý¼Óµµ(ÃÖ´ë) Áõ°¡
		{
			paramFactor.pfn_ENGINE_MAX_SPEED_UP+= fDestValue;
		}
		break;
	case DES_ENGINE_MIN_SPEED_UP:		// ¿£Áø ÀÏ¹Ý¼Óµµ(ÃÖ¼Ò) Áõ°¡
		{
	...
	case DES_ENGINE_GROUND_SPEED_UP:		// ¿£Áø Áö»ó¼Óµµ Áõ°¡
		...
		break;
	case DES_RADAR_OBJECT_DETECT_RANGE:		// ·¹ÀÌ´õ ¹°Ã¼ °¨Áö ¹Ý°æ
		{
	...
	case DES_PIERCE_UP_01:		// ±âº»¹«±â ÇÇ¾î½ºÀ² Áõ°¡ Ä«µå
		...
		break;
	case DES_PIERCE_UP_02:		// °í±Þ¹«±â ÇÇ¾î½ºÀ² Áõ°¡ Ä«µå
		{
	...
	case DES_SKILL_BUFF_MON_ATTACK_POWER:			// ¸ó½ºÅÍ °ø°Ý½Ã - °ø°Ý·Â Áõ°¡ : Value Áõ°¡ %
		{
			paramFactor.pfp_SKILL_BUFF_MON_ATTACK_POWER += fDestValue;
		}
		break;
	case DES_SKILL_BUFF_MON_ATTACK_PROBABILITY:		// ¸ó½ºÅÍ °ø°Ý½Ã - °ø°Ý·Â È®À² : Value Áõ°¡ %
		{
			paramFactor.pfp_SKILL_BUFF_MON_ATTACK_PROBABILITY += fDestValue;
		}
		break;
	case DES_SKILL_BUFF_MON_ATTACK_PIERCE:			// ¸ó½ºÅÍ °ø°Ý½Ã - ÇÇ¾î½º Áõ°¡ : Value Áõ°¡ %
		{
...
// 		break;
	case DES_DEFENSE_01	:				// ¹æ¾î·Â 01
		// 2011-09-26 by hsSon, È¸ÇÇ³ª ¹æ¾î ¾Æ¸Ó·ù¸¦ ÀåÂø Çß´Ù°¡ ¹þÀ¸¸é ³» Á¤º¸¿¡ È¸ÇÇ, ¹æ¾î°¡ »ó½ÂµÇ´Â ¹ö±× ¼öÁ¤
		paramFactor.pfp_DEFENSE_01 -= fDestValue;
		// end 2011-09-26 by hsSon, È¸ÇÇ³ª ¹æ¾î ¾Æ¸Ó·ù¸¦ ÀåÂø Çß´Ù°¡ ¹þÀ¸¸é ³» Á¤º¸¿¡ È¸ÇÇ, ¹æ¾î°¡ »ó½ÂµÇ´Â ¹ö±× ¼öÁ¤
		break;
	case DES_DEFENSE_02	:				// ¹æ¾î·Â 02
		// 2011-09-26 by hsSon, È¸ÇÇ³ª ¹æ¾î ¾Æ¸Ó·ù¸¦ ÀåÂø Çß´Ù°¡ ¹þÀ¸¸é ³» Á¤º¸¿¡ È¸ÇÇ, ¹æ¾î°¡ »ó½ÂµÇ´Â ¹ö±× ¼öÁ¤
		paramFactor.pfp_DEFENSE_02 -= fDestValue;
...
void ParamFactor_plus_ParamFactor( CParamFactor &destParamFactor, CParamFactor &srcParamFactor )
{
	...
	destParamFactor.pfn_ENGINE_MAX_SPEED_UP += srcParamFactor.pfn_ENGINE_MAX_SPEED_UP;	// ¿£Áø ÀÏ¹Ý¼Óµµ(ÃÖ´ë) Áõ°¡	
	destParamFactor.pfn_ENGINE_MIN_SPEED_UP += srcParamFactor.pfn_ENGINE_MIN_SPEED_UP;	// ¿£Áø ÀÏ¹Ý¼Óµµ(ÃÖ¼Ò) Áõ°¡
	...
	destParamFactor.pfn_RADAR_OBJECT_DETECT_RANGE += srcParamFactor.pfn_RADAR_OBJECT_DETECT_RANGE;	// ·¹ÀÌ´õ ¹°Ã¼ °¨Áö ¹Ý°æ
	destParamFactor.pfm_PIERCE_UP_01 += srcParamFactor.pfm_PIERCE_UP_01;	// ±âº»¹«±â ÇÇ¾î½ºÀ² Áõ°¡ Ä«µå
	destParamFactor.pfm_PIERCE_UP_02 += srcParamFactor.pfm_PIERCE_UP_02;	// °í±Þ¹«±â ÇÇ¾î½ºÀ² Áõ°¡ Ä«µå
...
{
	item.ReqGearStat.AttackPart += paramFactor.pfp_ATTACK_PART;// °ø°Ý µð¹ÙÀÌ½º ÆÄÆ®
	item.ReqGearStat.DefensePart += paramFactor.pfp_DEFENSE_PART;// ³»±¸ µð¹ÙÀÌ½º ÆÄÆ®
	item.ReqGearStat.FuelPart += paramFactor.pfp_FUEL_PART;// ¿¬·á µð¹ÙÀÌ½º ÆÄÆ®
	item.ReqGearStat.SoulPart += paramFactor.pfp_SOUL_PART;// °¨ÀÀ µð¹ÙÀÌ½º ÆÄÆ®
	item.ReqGearStat.ShieldPart += paramFactor.pfp_SHIELD_PART;// ¹æ¾î µð¹ÙÀÌ½º ÆÄÆ®
	item.ReqGearStat.DodgePart += paramFactor.pfp_DODGE_PART;	// È¸ÇÇ µð¹ÙÀÌ½º ÆÄÆ®
	item.SkillHitRate += paramFactor.pfp_SKILLPROBABILITY;// ½ºÅ³°ø°ÝÈ®·ü
...
//	item.AbilityMax += paramFactor.pfp_SPEED;// ÀÌµ¿¼Óµµ
	...
	if(IS_PRIMARY_WEAPON(item.Kind))
	{
		...
		item.AbilityMin *= (1.0f+paramFactor.pfm_MINATTACK_01);	// ÃÖ¼Ò °ø°Ý·Â 01, check: * 
		item.AbilityMax *= (1.0f+paramFactor.pfm_MAXATTACK_01);	// ÃÖ´ë °ø°Ý·Â 01, check: * 
		item.Defense += paramFactor.pfp_DEFENSE_01;	// ¹æ¾î·Â 01
		item.HitRate += paramFactor.pfp_ATTACKPROBABILITY_01;	// °ø°ÝÈ®·ü 01
		item.ReAttacktime *= (1.0f+paramFactor.pfm_REATTACKTIME_01);	// (*) ¸®¾îÅÃÅ¸ÀÓ 01 (- Áõ°¡)
		item.AbrasionRate += paramFactor.pfp_ABRASIONRATE_01;	// ¸¶¸ðÀ² 01
		...
		item.AttackTime += paramFactor.pfp_ATTACKTIME_01;// Ã³À½ °ø°Ý ½ÃÀÇ Å¸ÀÓ 01 (- Áõ°¡)
		item.Time *= (1.0f+paramFactor.pfm_TIME_01);	// (*) Áö¼Ó ½Ã°£ 01, check: + -> *
		item.ExplosionRange += paramFactor.pfp_EXPLOSIONRANGE_01;	// Æø¹ß¹Ý°æ
	...
	else if(IS_SECONDARY_WEAPON(item.Kind))
	{
		...
		item.AbilityMin *= (1.0f+paramFactor.pfm_MINATTACK_02);	// ÃÖ¼Ò °ø°Ý·Â 02, check: * 
		item.AbilityMax *= (1.0f+paramFactor.pfm_MAXATTACK_02);	// ÃÖ´ë °ø°Ý·Â 02, check: * 
		item.Defense += paramFactor.pfp_DEFENSE_02;	// ¹æ¾î·Â 02
		item.HitRate += paramFactor.pfp_ATTACKPROBABILITY_02;	// °ø°ÝÈ®·ü 02
		item.ReAttacktime *= (1.0f+paramFactor.pfm_REATTACKTIME_02);	// (*) ¸®¾îÅÃÅ¸ÀÓ 02 (- Áõ°¡)
		item.AbrasionRate += paramFactor.pfp_ABRASIONRATE_02;	// ¸¶¸ðÀ² 02
		...
		item.AttackTime += paramFactor.pfp_ATTACKTIME_02;	// Ã³À½ °ø°Ý ½ÃÀÇ Å¸ÀÓ 02 (- Áõ°¡)
		item.Time *= (1.0f+paramFactor.pfm_TIME_02);	// (*) Áö¼Ó ½Ã°£ 02, check: + -> *
		item.ExplosionRange += paramFactor.pfp_EXPLOSIONRANGE_02;	// Æø¹ß¹Ý°æ
...
//	case DES_GROUNDMODE	:				// Æø°Ý¸ðµå
//	case DES_SIEGEMODE	:				// ½ÃÁî¸ðµå
//	case DES_BEAMBULLET_01:				// (*) ºö¼Ò¸ðÃÑ¾Ë·® 01, check: Ãß°¡µÊ
//	case DES_BEAMBULLET_02	:			// (*) ºö¼Ò¸ðÃÑ¾Ë·® 02, check: Ãß°¡µÊ
//	case DES_PRIMARY_WEAPON	:			// 1Çü ¹«±â, ¹ü¿ë
//	case DES_SECONDARY_WEAPON:			// 2Çü ¹«±â, ¹ü¿ë
//	case DES_ALL_WEAPON	:				// 1,2Çü ¹«±â ¸ðµÎ, ¹ü¿ë
}
...
///				µÎÀÚ¸®·Î ÀÌ·ç¾îÁö´Âµ¥ ¾Õ¹øÈ£´Â À¯´ÖÄ«ÀÎµå, µÞ¹øÈ£´Â ¾÷±×·¹ÀÌµå·¹º§
/// \param		
/// \return		
///////////////////////////////////////////////////////////////////////////////
...
{
// 2005-11-25 by ispark, EP2¿¡¼­´Â ¹«±â Áö±ÞÀÌ °øÅëÀÌ¹Ç·Î Ç¥ÁØ¹«±â·Î ÀÔ·Â(1Çü : Å×ÀÏ)
...
/// \fn			
/// \brief		º¤ÅÍ°£ °¢À» ±¸ÇÑ´Ù. 
//#define RadToDeg	57.29577951f		 // ¶óµð¾ÈÀ» µµ·Î ¹Ù²Û´Ù.
//#define DegToRad	0.017453293f		 // µµ¸¦ ¶óµð¾ÈÀ¸·Î ¹Ù²Û´Ù.
...
/// \brief		Ä³¸¯ÅÍ ³ôÀÌ °¡Á®¿È (Ä«¸Þ¶ó LookAt, ID, ¸»Ç³¼± À§Ä¡)
/// \author		ispark
...
{
// 2007-07-20 by dgwoo ¸ðµç Ä³¸¯ÅÍÀÇ º¸ÆøÀ» °°°ÔÇÔ.
//	switch(nPilotNum) 
//	{
//	case 0:						// ¿©ÀÚ
//		return 50.0f;
...
{
	// 2007-12-21 by dgwoo Ã¢¸ðµå Áö¿ø
...
//				return 423;
//			// 2007-04-24 by bhsohn ¿ÍÀÌµå ¸ð´ÏÅÍ °ü·Ã Ã³¸®
//			else if(g_pD3dApp->GetBackBufferDesc().Height == 800)
//			{
//				return 311;
//			}
//			// 2007-08-23 by bhsohn 1280X720¿ÍÀÌµå ¸ð´ÏÅÍ °ü·Ã Ã³¸®
//			else if(720 == g_pD3dApp->GetBackBufferDesc().Height)
//			{
//				return 255;
//			}
//			// end 2007-08-23 by bhsohn 1280X720¿ÍÀÌµå ¸ð´ÏÅÍ °ü·Ã Ã³¸®
//			else
//				return 468;
//		}
...
BOOL EnableRevivalMap(USHORT nInfluenceType, USHORT nMapIndex)
{
	...
	{		
		...
		// 2013-03-04 by bhsohn ¸ð¼±/°ÅÁ¡Àü ¿öÇÁ°ü·Ã ¹ö±× ¼öÁ¤
		else if(MAP_INFLUENCE_ANI_BASE == pMapInfo->MapInfluenceType)
		{
			return FALSE;
		}		
		// END 2013-03-04 by bhsohn ¸ð¼±/°ÅÁ¡Àü ¿öÇÁ°ü·Ã ¹ö±× ¼öÁ¤
	}
	// ¾Ë¸µÅÏ
...
BOOL StringCullingUserDataEx(char *pLineString, int nCullStringNum, vector<string> *vecStorage, CD3DHanFont* pHanFont, BOOL bChRt /*= FALSE*/, BOOL bShop/*= FALSE*/)
{
	...
	BOOL bShopNextLine;				// ˜ÞÀ?¶§ ¾ÆÀÌÅÛ ÀÌ¸§ÀÌ ´ÙÀ½¶óÀÎ¿¡ °ÉÃÄÁ® ÀÖ½À´Ï±î ?
...
/// \author		// 2008-01-23 by bhsohn °ø¹éÀ¸·Î ±Û¾¾ Â¥¸£´Â ÇÔ¼ö »ý¼º
...
void	GetString_TimeLeft( DWORD nSec, char* pBuff )
{
	...
	else if( nSec >= ONEMIN_SEC )
	{
		...
		sprintf( pBuff, STRMSG_C_SKILL_0009, nNum );	// "%dºÐ"
	}
	else
	{
		nNum = nSec;
...
/// \brief		¸Þ´º ¹öÆ°°ú ¸ÞÀÎÃ¢¿¡ Ã¤ÆÃ ¹öÆ° ÀÎµ¦½º·Î º¯°æ.
/// \author		dgwoo
...
FLOAT GetAmorGearHeight(INT i_nAmmor, BOOL i_bSelect /* = FALSE*/)
{
	...
	// 2009. 07. 16 by ckPark ¼ö¹Ú¾Æ¸Ó ³ôÀÌ Á¶Àý
	case 8290600:
	case 8291600:
	case 8293600:
	case 8559400:
	...
	// end 2009. 07. 16 by ckPark ¼ö¹Ú¾Æ¸Ó ³ôÀÌ Á¶Àý
