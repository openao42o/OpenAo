...
CReportCtrl::CReportCtrl(): m_pWndEdit(NULL)
{
	...
	m_nSelItem = -1;	// 한개 선택한 아이템

	for(int i=0; i<MAX_COLUMN; ++i)
		m_bColumnEdit[i]	= TRUE;
}
...
/// 선택한 아이템
int	CReportCtrl::GetSelectedItem()
{
	const int ITEMS = CListCtrl::GetItemCount();
	int nCnt = 0;
	for(nCnt = 0;nCnt < ITEMS;nCnt++)
	{
		if(ExamItemStates(nCnt, RC_ITEM_FOCUSED | RC_ITEM_SELECTED))
		{
			return nCnt;
		}
	}
