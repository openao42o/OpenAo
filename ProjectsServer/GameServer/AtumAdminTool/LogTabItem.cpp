...
#define _STRMSG_S_SCAT_0100 QUERY_080825_0001
#define _STRMSG_S_SCAT_0101 QUERY_080825_0002
#define _STRMSG_S_SCAT_0102 QUERY_080825_0003
#define _STRMSG_S_SCAT_0103 QUERY_080825_0004
#define _STRMSG_S_SCAT_0104 QUERY_080825_0005
#define _STRMSG_S_SCAT_0105 QUERY_080825_0006
#define _STRMSG_S_SCAT_0106 QUERY_080825_0007
#define _STRMSG_S_SCAT_0107 QUERY_080825_0008
#define _STRMSG_S_SCAT_0108 QUERY_080825_0009

// 2012-11-21 by bckim, 운영툴기능추가, 인챈트아이템제거기능
#define _STRMSG_S_SCAT_0109 QUERY_130708_0001
#define _STRMSG_S_SCAT_0110 QUERY_130708_0002
// 2012-11-21 by bckim, 운영툴기능추가, 인챈트아이템제거기능. End
#define _STRMSG_S_SCAT_0111 QUERY_130708_0003		// 2013-07-01 by bckim, 10인챈트 미만 하이퍼카드처리, 로그추가, 운영툴 보완
	...
	: CDialog(CLogTabItem::IDD, pParent)
{
	...
	m_ctlbCheckRearrangeItemDeleted = FALSE;
	m_ctlbCheckRearrangeItemMain = FALSE;
	m_bCheckDisEnchantByAdmin = FALSE;	// 2012-11-21 by bckim, 운영툴기능추가, 인챈트아이템제거기능
	m_bCheckEnchantByAdmin = FALSE;	// 2012-11-21 by bckim, 운영툴기능추가, 인챈트아이템제거기능
	m_bCheckTryEnchantInfo	= FALSE;		// 2013-07-01 by bckim, 10인챈트 미만 하이퍼카드처리, 로그추가, 운영툴 보완
	//}}AFX_DATA_INIT
...
void CLogTabItem::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_DISENCHANT_BY_ADMIN, m_bCheckDisEnchantByAdmin);	// 2012-11-21 by bckim, 운영툴기능추가, 인챈트아이템제거기능
	DDX_Check(pDX, IDC_CHECK_ENCHANT_BY_ADMIN, m_bCheckEnchantByAdmin);	// 2012-11-21 by bckim, 운영툴기능추가, 인챈트아이템제거기능
	DDX_Check(pDX, IDC_CHECK_TRY_ENCHANT_INFO, m_bCheckTryEnchantInfo);		// 2013-07-01 by bckim, 10인챈트 미만 하이퍼카드처리, 로그추가, 운영툴 보완
	//}}AFX_DATA_MAP
...
BEGIN_MESSAGE_MAP(CLogTabItem, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_SELECT_ITEM, OnButtonSelectItem)
...
void CLogTabItem::OnButtonOk() 
{
	...
	// ENCHANT 
	if (bRet && m_bCheckEnchantByAdmin)
	{
		bRet = GetLogAndInsertToGrid(T1_FL_LOG_ITEM_ENCHANT_BY_ADMIN, "atum_log_item_Enchant_Change_By_Admin",
			_STRMSG_S_SCAT_0109 );
		if(bRet)
		{
			bRet = GetLogAndInsertToGrid(T1_FL_LOG_ITEM_ENCHANT_BY_ADMIN, "atum_backup_log_item_Enchant_Change_By_Admin"
				, _STRMSG_S_SCAT_0109 );
		}
	}

	// DISENCHANT
	if (bRet && m_bCheckDisEnchantByAdmin)
	{
		bRet = GetLogAndInsertToGrid(T1_FL_LOG_ITEM_DISENCHANT_BY_ADMIN, "atum_log_item_Enchant_Change_By_Admin",
			_STRMSG_S_SCAT_0110 );
		if(bRet)
		{
			bRet = GetLogAndInsertToGrid(T1_FL_LOG_ITEM_DISENCHANT_BY_ADMIN, "atum_backup_log_item_Enchant_Change_By_Admin"
				, _STRMSG_S_SCAT_0110 );
		}
	}
#endif
	...
	if (bRet && m_bCheckTryEnchantInfo)
	{
		bRet = GetLogAndInsertToGrid(T1_FL_LOG_ITEM_TRY_ENCHANT_INFO, "atum_log_item_Try_Enchant_Info"
			,_STRMSG_S_SCAT_0111 );
		if(bRet)
		{
			bRet = GetLogAndInsertToGrid(T1_FL_LOG_ITEM_TRY_ENCHANT_INFO, "atum_backup_log_item_Try_Enchant_Info"
				, _STRMSG_S_SCAT_0111 );
		}
	}
...
BOOL CLogTabItem::GetLogAndInsertToGrid(T1<T0::FL_LOG> T1_LogType, LPCSTR szTableName, LPCSTR szContentString)
{
	...
	if (m_bCheckMaxRowCount)
	{
		...
		szQuery.Format(QUERY_080702_0012_1_A, min(m_nMaxRowCount, SIZE_MAX_LOG_ROW_COUNT));		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
	else
	{
		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		//szQuery = "select ";
		szQuery = QUERY_080702_0012_1_B;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
...
// 	szQuery += " l, ti_item ii, td_character c WITH (NOLOCK) where i.logtype = l.logtype and l.itemnum = ii.itemnum and c.uniquenumber = l.characteruniquenumber ";
	...
	szQuery += QUERY_080702_0012_2;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	szQuery += szContentString;
	szQuery += QUERY_080702_0012_3;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	szQuery += szTableName;
	szQuery += QUERY_080702_0012_4;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 

	// log type
	CString szLogTypeCond;
	// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	//szLogTypeCond.Format(" and l.logtype = %d ", T1_LogType);
	szLogTypeCond.Format(QUERY_080702_0012_5, T1_LogType);			// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	...
	if (m_bCheckAccountName)
	{
		...
		if (!m_bCheckAccountName2)
		{
			...
			szAccountCond.Format(QUERY_080702_0012_6_A, m_szAccountName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		}
		else
		{
			// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
			//szAccountCond.Format(" and (c.accountname = \'%s\' or c.accountname = \'%s\') ", m_szAccountName, m_szAccountName2);
			szAccountCond.Format(QUERY_080702_0012_6_B, m_szAccountName, m_szAccountName2);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		}
	...
	if (m_bCheckCharacterName)
	{
		...
		)
		{
			...
			szCharacterCond.Format(QUERY_080702_0012_7_A,		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		...
		else
		{
			...
			szCharacterCond.Format(QUERY_080702_0012_7_B, m_szCharacterName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		}
	...
	if (m_bCheckDate)
	{
		...
		szDateCond.Format(QUERY_080702_0012_8,		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
			(LPCSTR)(m_dateStart.Format("%Y/%m/%d") + m_timeStart.Format(" %H:%M:%S.0")),
			(LPCSTR)(m_dateEnd.Format("%Y/%m/%d") + m_timeEnd.Format(" %H:%M:%S.0")));
		szQuery += szDateCond;
	}
	...
	if (m_bCheckItemName)
	{
		...
		szItemNum.Format(QUERY_080702_0012_9, m_nSelectedItemNum);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		szQuery += szItemNum;
	}
	...
	if (m_bCheckItemUID)
	{
		...
		szItemUID.Format(QUERY_080702_0012_10, m_nItemUID);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		szQuery += szItemUID;
	}
	...
	szQuery += QUERY_080702_0012_11;			// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	...
	{
		char szTm[1024];
		sprintf(szTm, QUERY_080702_0012_12_A, min(m_nMaxRowCount, SIZE_MAX_LOG_ROW_COUNT));
		szQuery += szTm;
	}
	else
	{
		szQuery += QUERY_080702_0012_12_B;
	}
#endif
	...
	int ItemUID;
	char Contents[512];		// 2013-07-01 by bckim, 10인챈트 미만 하이퍼카드처리, 로그추가, 운영툴 보완  200->512
...
//	if (!m_pODBCStmt->ExecuteQuery(szQuery))
	...
	cb7 = SQL_NTS;
	SQLBindCol(m_pODBCStmt->m_hstmt, 8, SQL_C_CHAR, Contents, 512, &cb8);		// 2013-07-01 by bckim, 10인챈트 미만 하이퍼카드처리, 로그추가, 운영툴 보완 200->512 수정 
...
void CLogTabItem::OnButtonSelectAll() 
{
	...
	m_bCheckDisEnchantByAdmin	= TRUE;	// 2012-11-21 by bckim, 운영툴기능추가, 인챈트아이템제거기능
	m_bCheckEnchantByAdmin		= TRUE;	// 2012-11-21 by bckim, 운영툴기능추가, 인챈트아이템제거기능

	m_bCheckTryEnchantInfo		= TRUE;	// 2013-07-01 by bckim, 10인챈트 미만 하이퍼카드처리, 로그추가, 운영툴 보완
