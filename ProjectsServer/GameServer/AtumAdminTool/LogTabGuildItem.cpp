	...
	: CDialog(CLogTabGuildItem::IDD, pParent)
{
	...
	m_bCheckDate = TRUE;
	m_bCheckDrawGuildStore = TRUE;	// 2013-04-01 by bckim, 캐나다 운영툴 편의성 보완
	m_bCheckGuildUID = FALSE;
	m_bCheckGuildName = FALSE;
	m_bCheckInsertGuildStore = TRUE;	// 2013-04-01 by bckim, 캐나다 운영툴 편의성 보완
...
BOOL CLogTabGuildItem::GetLogAndInsertToGrid(T1<T0::FL_LOG> T1_LogType, LPCSTR szTableName, LPCSTR szContentString)
{
	...
	if (m_bCheckMaxRowCount)
	{
		...
		szQuery.Format(QUERY_080702_0011_1_A, min(m_nMaxRowCount, SIZE_MAX_LOG_ROW_COUNT));		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
	else
	{
		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		//szQuery = "select ";
		szQuery = QUERY_080702_0011_1_B;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
...
// 	szQuery += " l WITH (NOLOCK) where i.logtype = l.logtype ";
	...
	szQuery += QUERY_080702_0011_2;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	szQuery += szContentString;
	szQuery += QUERY_080702_0011_3;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	szQuery += szTableName;
	szQuery += QUERY_080702_0011_4;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 

	// log type
	CString szLogTypeCond;
// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
//	szLogTypeCond.Format(" and l.logtype = %d ", T1_LogType);
	szLogTypeCond.Format(QUERY_080702_0011_5, T1_LogType);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	...
	{// account
		...
		temp.Format(QUERY_080702_0011_6, m_szAccountName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		szQuery += temp;
	}
	...
	{// character
		...
		temp.Format(QUERY_080702_0011_7, m_szCharacterName);	// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		szQuery += temp;
	}
	...
	{// date
		...
		temp.Format(QUERY_080702_0011_8,		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
			(LPCSTR)(m_StartDate.Format("%Y/%m/%d") + m_StartTime.Format(" %H:%M:%S.0")),
			(LPCSTR)(m_EndDate.Format("%Y/%m/%d") + m_EndTime.Format(" %H:%M:%S.0")));
		szQuery += temp;
	}
	...
	{// GuildName
		...
		temp.Format(QUERY_080702_0011_9, m_szGuildName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		szQuery += temp;
	}
	...
	{// GuildUID
		...
		temp.Format(QUERY_080702_0011_10, m_nGuildUID);			// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		szQuery += temp;
	}
	...
	{// ItemNum
		...
		temp.Format(QUERY_080702_0011_11, m_nItemNum);			// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		szQuery += temp;
	}
	...
	{// ItemUID
		...
		temp.Format(QUERY_080702_0011_12, m_nItemUID);			// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		szQuery += temp;
	}
	...
	szQuery += QUERY_080702_0011_13;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
#if defined(DB_SERVER_MYSQL)
	// 2008-07-08 by cmkwon, MySQL 지원 구현 - 
	if(m_bCheckMaxRowCount)
	{
		char szTm[1024];
		sprintf(szTm, QUERY_080702_0011_14_A, min(m_nMaxRowCount, SIZE_MAX_LOG_ROW_COUNT));
		szQuery += szTm;
	}
	else
	{
		szQuery += QUERY_080702_0011_14_B;
	}
#endif
...
//	BOOL bRet = m_pODBCStmt->ExecuteQuery(szQuery);
	...
	// 2012-12-11 by khkim ChangeCount 모두 0으로 나오는 문제 수정 (ItemName테이블이 없으므로 밀린것임)   
	//SQLBindCol(m_pODBCStmt->m_hstmt, 11, SQL_C_CHAR		, GuilditemInfo.ItemName		, SIZE_MAX_ITEM_NAME	, &arrCB[11]);
	SQLBindCol(m_pODBCStmt->m_hstmt, 11, SQL_C_ULONG	, &GuilditemInfo.PrefixCodeNum	, 0						, &arrCB[11]);
	SQLBindCol(m_pODBCStmt->m_hstmt, 12, SQL_C_ULONG	, &GuilditemInfo.SuffixCodeNum	, 0						, &arrCB[12]);
	SQLBindCol(m_pODBCStmt->m_hstmt, 13, SQL_C_ULONG	, &GuilditemInfo.ItemResultCount, 0						, &arrCB[13]);
	SQLBindCol(m_pODBCStmt->m_hstmt, 14, SQL_C_LONG		, &GuilditemInfo.ItemChangeCount, 0						, &arrCB[14]);
	// 2012-12-11 End by khkim ChangeCount
