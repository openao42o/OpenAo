...
#define _STRMSG_S_SCAT_0122 "'Change EXP: ' + CAST(l.Param1 AS VARCHAR(15)) + ', Total EXP: ' + CAST(l.Param2 AS VARCHAR(15))"
...
BOOL CLogTabUser::GetLogAndInsertToGrid(T1<T0::FL_LOG> T1_LogType, LPCSTR szTableName, LPCSTR szContentString1, LPCSTR szContentString2 /* = NULL */)
{
	...
	if (m_bCheckMaxRowCount)
	{
		...
		szQuery.Format(QUERY_080702_0095_1_A, min(m_nMaxRowCount, SIZE_MAX_LOG_ROW_COUNT));		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
	else
	{
		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		//szQuery = "select ";
		szQuery = QUERY_080702_0095_1_B;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
		...
		|| stricmp("atum_backup_log_user_game_start_end", szTableName) == 0)
	{
		...
		szQuery += QUERY_080702_0095_2_A;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
	else
	{
		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		//szQuery += " c.accountname, c.charactername, l.Time, i.LogStringKO, l.MapIndex, l.Position_X, l.Position_Y, l.Position_Z, ";
		szQuery += QUERY_080702_0095_2_B;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
	...
	szQuery += QUERY_080702_0095_3;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		...
		|| stricmp("atum_backup_log_user_game_start_end", szTableName) == 0)
	{
		...
		szQuery += QUERY_080702_0095_4_A;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
	else
	{
		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		//szQuery += " l, td_character c WITH (NOLOCK) where i.logtype = l.logtype and c.uniquenumber = l.characteruniquenumber ";
		szQuery += QUERY_080702_0095_4_B;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	}
	...
	szLogTypeCond.Format(QUERY_080702_0095_5, T1_LogType);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	...
	if (m_bCheckAccountName)
	{
			...
			|| stricmp("atum_backup_log_user_game_start_end", szTableName) == 0)
		{
			...
			szAccountCond.Format(QUERY_080702_0095_6_A, m_szAccountName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		}
		else
		{
			// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
			//szAccountCond.Format(" and c.accountname = \'%s\' ", m_szAccountName);
			szAccountCond.Format(QUERY_080702_0095_6_B, m_szAccountName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		}
	...
	if (m_bCheckCharacterName)
	{
			...
			|| stricmp("atum_backup_log_user_game_start_end", szTableName) == 0)
		{
			...
			szCharacterCond.Format(QUERY_080702_0095_7_A, m_szCharacterName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		}
		else
		{
			// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
			//szCharacterCond.Format(" and c.charactername = \'%s\' ", m_szCharacterName);
			szCharacterCond.Format(QUERY_080702_0095_7_B, m_szCharacterName);	// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
		}
	...
	if (m_bCheckDate)
	{
		...
		szDateCond.Format(QUERY_080702_0095_8,		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
						(LPCSTR)(m_dateStart.Format("%Y/%m/%d") + m_TimeStart.Format(" %H:%M:%S.0")),
						(LPCSTR)(m_dateEnd.Format("%Y/%m/%d") + m_TimeEnd.Format(" %H:%M:%S.0")));
		szQuery += szDateCond;
	}
	...
	szQuery += QUERY_080702_0095_9;		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
#if defined(DB_SERVER_MYSQL)
	// 2008-07-08 by cmkwon, MySQL 지원 구현 - 
	if(m_bCheckMaxRowCount)
	{
		char szTm[1024];
		sprintf(szTm, QUERY_080702_0095_10_A, min(m_nMaxRowCount, SIZE_MAX_LOG_ROW_COUNT));
		szQuery += szTm;
	}
	else
	{
		szQuery += QUERY_080702_0095_10_B;
	}
#endif
...
//	if (!m_pODBCStmt->ExecuteQuery(szQuery))
	...
	while ((sqlReturn = SQLFetch(m_pODBCStmt->m_hstmt)) != SQL_NO_DATA)
	{
		...
		{
			m_pODBCStmt->ProcessLogMessagesForStmt(PROCEDURE_080827_0010);	// 2008-12-09 by cmkwon, 수정함.
		}
