...
//-----------------------------------------------------------------------------
// Global access to the app (needed for the global WriteSystemLog())
//-----------------------------------------------------------------------------
#include "NPCGlobal.h"
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::Create( HINSTANCE hInstance, HWND hWnd)
{
    HRESULT hr;
    ...
    {
        util::release( m_pD3D );
		return DisplayErrorMsg( hr, MSGERR_APPMUSTEXIT );		// 2008-09-02 by cmkwon, NPC Server 몬스터 충돌 처리 여부 메시지 박스 띄우기
// 2008-09-02 by cmkwon, NPC Server 몬스터 충돌 처리 여부 메시지 박스 띄우기
//		return hr;
// 2004-11-02 by cmkwon, NPC Server는 메세지 박스를 띄우지 않도록 수정
...
HRESULT CD3DApplication::BuildDeviceList()
{
    ...
    for( UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++ )
    {
        ...
        m_pD3D->GetAdapterIdentifier( iAdapter, 0 /*D3DENUM_NO_WHQL_LEVEL*/, &pAdapter->d3dAdapterIdentifier );
        ...
        DWORD dwNumModes        = 0;
        DWORD dwNumAdapterModes = m_pD3D->GetAdapterModeCount( iAdapter , D3DFMT_X8R8G8B8 ); // panoskj as of d3d9 we enumerate only 32bit modes
        ...
        for( UINT iDevice = 0; iDevice < dwNumDeviceTypes; iDevice++ )
        {
            ...
            for( DWORD f=0; f<dwNumFormats; f++ )
            {
                ...
                if( pDevice->DeviceType == D3DDEVTYPE_HAL )
                {
                    ...
                    if( true ) // all devices supporting d3d9 can handle this // pDevice->d3dCaps.Caps2 & D3DCAPS2_CANRENDERWINDOWED )
                    {
            ...
            if( bFormatConfirmed[0] ) // all devices can // && (pDevice->d3dCaps.Caps2 & D3DCAPS2_CANRENDERWINDOWED) )
            {
...
//		DisplayErrorMsg( D3DAPPERR_NULLREFDEVICE, 0 );
    ...
    // Create the device
	pModeInfo->dwBehavior	|= D3DCREATE_MULTITHREADED;	// 2009-02-09 by cmkwon, D3D 인스턴스 생성시 MutiThread 옵션 추가
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
    ...
    if( MSGERR_APPMUSTEXIT == dwType )
    {
		...
		_tcscat( strMsg, _T(" This sample will now exit.\r\n") );
		g_pNPCGlobal->WriteSystemLog ( strMsg );
    ...
    else
    {
		...
		if( MSGWARN_SWITCHEDTOREF == dwType )
            _tcscat( strMsg, _T(" Switching to the reference rasterizer, ")
							 _T("a software device that implements the entire ")
                             _T("Direct3D feature set, but runs very slowly.\r\n") );
		g_pNPCGlobal->WriteSystemLog ( strMsg );
    }
