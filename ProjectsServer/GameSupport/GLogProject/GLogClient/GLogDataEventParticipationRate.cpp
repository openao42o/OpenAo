...
{
	util::zero(m_arrEventParticipationRateData,sizeof(CEventParticipationRateData)*GLOG_DATA_MAX_CNT);
}
...
Err_t CGLogDataEventParticipationRate::GLogDBSelect(int * o_pSelectCnt)
{
	...
	while(!m_pSourceRecordSet->EndOfFile)
	{
		...
		util::zero(&tmpEventParticipationRateData,sizeof(CEventParticipationRateData));
