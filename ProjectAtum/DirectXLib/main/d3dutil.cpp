...
VOID D3DUtil_InitMaterial( D3DMATERIAL9& mtrl, FLOAT r, FLOAT g, FLOAT b,
                           FLOAT a )
{
    ZeroMemory( &mtrl, sizeof(D3DMATERIAL9) ); // 2015-10-21 PanKJ when firing missiles this whole function gets called too many times
...
//       colors. It does not set emissive or specular colors.
//-----------------------------------------------------------------------------
VOID D3DUtil_InitMaterial( D3DMATERIAL9& mtrl, unsigned char r, unsigned char g, unsigned char b,
                           unsigned char a )
{
    ZeroMemory( &mtrl, sizeof(D3DMATERIAL9) );
	mtrl.Diffuse.r = mtrl.Ambient.r = ((float)r) / 255;
    mtrl.Diffuse.g = mtrl.Ambient.g = ((float)g) / 255;
    mtrl.Diffuse.b = mtrl.Ambient.b = ((float)b) / 255;
    mtrl.Diffuse.a = mtrl.Ambient.a = ((float)a) / 255;
}
...
End:
    ...
    if( hdcMask != NULL )
        DeleteDC( hdcMask );
    util::del_array( pcrArrayColor );
    util::del_array( pcrArrayMask );
...
CD3DCamera::CD3DCamera()
{
    ...
    SetProjParams( D3DX_PI/3, 1.333f, 1.0f, 1000.0f );
}
...
// Desc:
//-----------------------------------------------------------------------------
void CD3DCamera::SetViewParams(const D3DXVECTOR3 &vEyePt, const D3DXVECTOR3& vLookatPt, const D3DXVECTOR3& vUpVec )
{
	// Set attributes for the view matrix
	m_vEyePt = vEyePt;
	m_vLookatPt = vLookatPt;
	m_vUpVec = vUpVec;
	auto vDir = m_vLookatPt - m_vEyePt;
	...
	D3DXMatrixInverse(&m_matBillboard, nullptr, &m_matView);
