...
#include <stdarg.h>
#include "DXUtil.h"
// 2012-12-06 by bhsohn 장기간 window 킨후, 게임접속시 게임 멈추어 있는 현상 처리
#include "Contents.h"
#include "DbgOut_C.h"	
// END 2012-12-06 by bhsohn 장기간 window 킨후, 게임접속시 게임 멈추어 있는 현상 처리
...
FLOAT __stdcall DXUtil_Timer( TIMER_COMMAND command )
{
	...
	// 2011. 10. 27 by hsson CPU오버 클럭(EPU-6)으로 DXUtil_Timer 함수가 정상보다 더 큰 값이 리턴 되던 버그 수정
	m_bUsingQPF = FALSE;
	// end 2011. 10. 27 by hsson CPU오버 클럭(EPU-6)으로 DXUtil_Timer 함수가 정상보다 더 큰 값이 리턴 되던 버그 수정
	...
	// 2012-12-06 by bhsohn 장기간 window 킨후, 게임접속시 게임 멈추어 있는 현상 처리
#ifdef C_LONGTIME_WINDOW_ON
    else	
	{
        // Get the time using timeGetTime()
        static DWORD m_dwLastElapsedTime  = 0;
        static DWORD m_dwBaseTime         = 0;
        static DWORD m_dwStopTime         = 0;
        DWORD dwTime=0;
        double fElapsedTime=0.0;
        
        // Get either the current time or the stop time, depending
        // on whether we're stopped and what command was sent
        if( m_dwStopTime != 0 && command != TIMER_START && command != TIMER_GETABSOLUTETIME)
		{
			dwTime = m_dwStopTime;
		}
        else
        ...
        if( command == TIMER_GETELAPSEDTIME )
        {   
			if(dwTime < m_dwLastElapsedTime)
			{
				return (FLOAT)fElapsedTime;
			}
			DWORD dwDiff = dwTime - m_dwLastElapsedTime;
            fElapsedTime =  (double)dwDiff * 0.001;
        ...
        if( command == TIMER_GETAPPTIME )
        {
			if(dwTime < m_dwBaseTime)
			{
				return (FLOAT)fElapsedTime;
			}

			DWORD dwDiff = dwTime - m_dwBaseTime;
			fElapsedTime =  (double)dwDiff * 0.001;
            return (FLOAT) (fElapsedTime);
        }
        ...
        if( command == TIMER_START )
        {
            if( m_bTimerStopped )
			{
				m_dwBaseTime += dwTime - m_dwStopTime;
			}
            m_dwStopTime = 0;
            m_dwLastElapsedTime  = dwTime;
            m_bTimerStopped = FALSE;
        ...
        if( command == TIMER_ADVANCE )
        {
            m_dwStopTime += 100; // (100ms)
            return 0.0f;
        }
		
        if( command == TIMER_GETABSOLUTETIME )
        {
			fElapsedTime =  (double)dwTime * 0.001;
            return (FLOAT) fElapsedTime;
        }
